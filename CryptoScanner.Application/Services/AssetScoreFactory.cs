...
public static class AssetScoreFactory
{
    public static AssetScore Create(AssetAnalysis analysis, string marketRegime, IReadOnlySet<string> favoriteSymbols, EligibilityThresholds? thresholds = null) => new()
    {
        ...
        Resistance = analysis.Direction == TradeDirection.Short ? analysis.Risk.Support : analysis.Risk.Resistance,
        TakeProfit1 = analysis.Risk.TakeProfit1,
        TakeProfit3 = analysis.Risk.TakeProfit3,
        VolumeSpike = analysis.Volume.Spike,
        ResistanceDistance = analysis.Direction == TradeDirection.Short ? analysis.Risk.SupportDistancePercent : analysis.Risk.ResistanceDistancePercent,
        SupportDistance = analysis.Direction == TradeDirection.Short ? analysis.Risk.ResistanceDistancePercent : analysis.Risk.SupportDistancePercent,
        ...
        IsEligible = EligibilityEvaluator.Evaluate(analysis, marketRegime, thresholds, analysis.Direction).IsEligible,
        ...
        Support = analysis.Direction == TradeDirection.Short ? analysis.Risk.Resistance : analysis.Risk.Support,
